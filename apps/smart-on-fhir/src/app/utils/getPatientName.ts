...
export const getPatientName = (patient?: IPatient) => {
  if (patient && patient.name) {
    const nameObject = patient.name.find(
      (elem) => elem.use === HumanNameUseKind._official
    );
    let fullName: string = '';
  ...
  return 'Ola Nordmann Grytelokk';  //Returns a standard name when testing without SMART launcher
};
